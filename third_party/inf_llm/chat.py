...
def chat_loop(
    ...
    if inf_llm_config is not None:
        patch_type = inf_llm_config.get("type")
        if patch_type is None:
            warnings.warn("InfLLM config provided without `type`; skipping patch.")
        else:
            model = patch_hf(model, patch_type, **inf_llm_config)
...
def main(args):
    ...
    if args.inf_llm_config_path is not None:
        ...
        conv_type = inf_llm_config.get("conv_type", None)
        if conv_type == "llama-3-inst":
            args.conv_template = "llama-3-inst"

        model_config = inf_llm_config.get("model", None)
        inf_llm_config = model_config if model_config is not None else None
    else:
        inf_llm_config = None
